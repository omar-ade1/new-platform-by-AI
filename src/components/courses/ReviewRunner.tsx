...
type Question = { id: string; passage: Passage | null; options: Option[]; question_text: string };
type SubmitResult = { attempt_id: string; score: number; total_questions: number; review: ReviewQuestion[] };

type Phase = "loading" | "empty" | "in_progress" | "submitting" | "submitted";
...
export default function ReviewRunner({
  ...
  async function loadWrongQuestions() {
    ...
    setQuestions(list);
    setPhase(list.length === 0 ? "empty" : "in_progress");
  }
  ...
  // لما النتيجة تظهر، نرجّع الصفحة لفوق بدل ما تفضل واقفة في نص/آخر الأسئلة
  useEffect(() => {
  ...
  async function submitAttempt() {
    ...
    const { data, error } = await supabase.rpc("submit_review_attempt", { p_answers: payload });

    submittingRef.current = false;

    if (error || !data) {
      toast.error("حصل خطأ في تسليم المراجعة");
  ...
  if (phase === "loading") {
    return (
      <div className="bg-surface rounded-2xl border-2 border-ink/10 p-8 text-center">
        <p className="text-ink/50 text-base">جاري تحميل أسئلتك الغلط...</p>
      </div>
    );
  }
  ...
  if (phase === "empty") {
    return (
      <div className="bg-surface rounded-2xl border-2 border-ink/10 p-8 text-center space-y-3">
        <div className="w-16 h-16 rounded-2xl bg-teal/10 text-teal flex items-center justify-center mx-auto">
          <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8">
            <path d="M20 6 9 17l-5-5" />
          </svg>
        </div>
        <p className="font-display font-bold text-lg">مفيش أسئلة غلط محتاجة مراجعة دلوقتي 🎉</p>
        <p className="text-ink/50 text-base">{scopeLabel}</p>
      </div>
    );
  }
      ...
      <div className="space-y-5">
        <div className="sticky top-2 z-10 flex items-center justify-between gap-3 bg-surface rounded-2xl border-2 border-ink/10 px-5 py-3.5 shadow-lg shadow-primary/5">
          <p className="text-base font-bold text-ink/60">
            اتجاوبت {answeredCount} من {totalQuestions}
          </p>
          <span className="text-sm font-bold text-primary bg-primary/10 rounded-full px-3 py-1">{scopeLabel}</span>
        </div>
        ...
        >
          {phase === "submitting" ? "جاري التسليم..." : "سلّم المراجعة"}
        </button>
        ...
        <AnimatePresence>
          {confirmOpen && (
            ...
            >
              ...
              >
                <h2 className="font-display font-black text-2xl text-primary mb-2">تسليم المراجعة</h2>
                <p className="text-ink/60 text-base mb-6">
                  {unansweredCount > 0
                    ? `لسه فيه ${unansweredCount} سؤال من غير إجابة. متأكد عايز تسلّم كده؟`
                    : "متأكد عايز تسلّم المراجعة؟"}
                </p>
  ...
  return (
    <div className="space-y-5">
      <div className="bg-surface rounded-2xl border-2 border-ink/10 p-8 text-center space-y-2">
        <p className="text-ink/50 text-base font-bold">نتيجتك في المراجعة</p>
        ...
        <button
          ...
          className="mt-4 px-7 py-3 rounded-full border-2 border-primary/20 text-primary font-display font-bold text-base hover:bg-primary/5 transition-colors"
        >
          جرب اللي لسه غلط تاني
        </button>
      </div>
